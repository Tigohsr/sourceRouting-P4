...
def main():
    ...
    s = net.get('s2')
    sw_mac_s2_eth1 = s.intf("s2-eth2").MAC()
    h = net.get('h2')
    h.setARP(sw_addr[1], sw_mac_s2_eth1)
    h.setDefaultRoute("dev eth0 via %s" % sw_addr[1])

    s = net.get('s3')
    sw_mac_s3_eth1 = s.intf("s3-eth2").MAC()
    h = net.get('h3')
    h.setARP(sw_addr[2], sw_mac_s3_eth1)
    h.setDefaultRoute("dev eth0 via %s" % sw_addr[1])
#
#    s = net.get('s4')
...
#    h.setDefaultRoute("dev eth0 via %s" % sw_addr[2])
    ...
    for i in xrange(nb_switches):
		...
		print ("Debugging switch match-action table")
                print output
